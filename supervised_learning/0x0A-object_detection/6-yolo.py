    ...
    def show_boxes(self, image, boxes, box_classes, box_scores, file_name):
        ...
        for i in range(len(box_scores)):
            ...
            cv2.rectangle(image, (int(boxes[i, 0]), int(boxes[i, 1])),
                          ...
                          (255, 0, 0), 2)
            cv2.putText(image, "{} {}".format(self.class_names[box_classes[i]], round(box_scores[i], 2), (255, 0, 0)),
                        (int(boxes[i, 0]), int(boxes[i, 1] - 5)), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255),
                        1, cv2.LINE_AA)
        try:
