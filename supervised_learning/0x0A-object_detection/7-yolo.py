...
class Yolo():
    ...
    def show_boxes(self, image, boxes, box_classes, box_scores, file_name):
        ...
        for i in range(len(box_scores)):
            cv2.rectangle(image, (int(boxes[i, 0]), int(boxes[i, 1])),
                          ...
                          (255, 0, 0), 2)
            cv2.putText(image, "{} {}".format(self.class_names
                                              [box_classes[i]],
                        round(box_scores[i], 2), (255, 0, 0)),
    ...
    def predict(self, folder_path):
        ...
        outputs_img = self.model.predict(pimages)
        fst, snd, trd = outputs_img

        for i in range(len(images)):
            boxes, box_confidences, box_class_probs = self.process_outputs(
                [fst[i], snd[i], trd[i]], image_shapes[i])
            filtered_boxes, box_classes, box_scores = self.filter_boxes(
                boxes, box_confidences, box_class_probs)
            box_p, p_box_classes, p_box_scores = self.non_max_suppression(
                filtered_boxes, box_classes, box_scores)
            self.show_boxes(images[i], box_p, p_box_classes,
                            p_box_scores, image_paths[i].split('/')[-1])
            predictions.append((box_p, p_box_classes, p_box_scores))
        return predictions, image_paths
